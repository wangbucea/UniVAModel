...
class TrajectoryAutoregressiveModel(nn.Module):
    ...
    def encode_input_sequence(self, action_sequence):
        """
        对输入的完整动作序列进行编码
        Args:
            action_sequence: [B, seq_len, action_dim] 输入动作序列
        Returns:
            encoded_sequence: [B, seq_len, feature_dim] 编码后的序列特征
        """
        B = action_sequence.shape[0]
        
        # 动作嵌入
        action_emb = self.action_embed(action_sequence)  # [B, seq_len, feature_dim]
        
        # 位置编码
        pos_emb = self.pos_embed.unsqueeze(0).repeat(B, 1, 1)  # [B, seq_len, feature_dim]
        
        # 序列编码
        sequence_input = action_emb + pos_emb
        encoded_sequence = self.sequence_encoder(sequence_input)
        
        return encoded_sequence
        
    def forward(self, action_sequence, seg_features, det_features, Vfeatures, point_features, target_actions=None):
        """
        Args:
            action_sequence: [B, seq_len, action_dim] 输入动作序列
        ...
        # 1. 编码输入动作序列
        encoded_input = self.encode_input_sequence(action_sequence)  # [B, seq_len, feature_dim]
        ...
        if self.training and target_actions is not None:
            ...
            # 创建因果掩码
            tgt_mask = self._generate_square_subsequent_mask(self.seq_len).to(action_sequence.device)
...
class UnifiedTransformerHead(nn.Module):
    ...
    def __init__(self, 
        ...
        # 参考点初始化
        self.reference_points = nn.Linear(feature_dim, 2)
