...
class RobotUniADModel224(nn.Module):
    ...
    def forward(self, images, input_actions=None, target_actions=None, det_targets=None):
        """
        Args:
            images: [B, T, C, H, W] 输入图像序列，H=W=224
...
if __name__ == "__main__":
    ...
    batch_size = 1
    timesteps = 1
    images = torch.randn(batch_size, timesteps, 3, 224, 224)
    images2 = torch.randn(batch_size, timesteps, 3, 224, 224)
    ...
    with torch.no_grad():
        ...
        # 验证分割输出尺寸
        print(f"\n输入图像尺寸: {images[0].shape[2:]}")
        print(f"分割输出尺寸: {outputs['seg_logits'].shape[2:]}")
        print(f"尺寸匹配: {outputs['seg_logits'].shape[2:] == images[0].shape[2:]}")
