...
class FeatureHeatmapValidator:
    ...
    def _create_summary_report(self, all_stats):
        ...
        axes[0, 0].set_title('Average Feature Means')
        ...
        # 特征形状信息
        shape_info = "Feature Shapes:\n"
        for name in names:
            shape_info += f"{name}: {summary[name]['shape']}\n"
        
        axes[1, 0].text(0.05, 0.95, shape_info, transform=axes[1, 0].transAxes,
                        fontsize=10, verticalalignment='top', fontfamily='monospace')
        axes[1, 0].set_title('Feature Shapes')
        axes[1, 0].axis('off')
